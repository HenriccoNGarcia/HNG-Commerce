...
class HNG_User_Profile {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_hng_send_password_2fa_code', [$this, 'ajax_send_password_2fa_code']);
        add_action('wp_ajax_hng_verify_password_2fa_code', [$this, 'ajax_verify_password_2fa_code']);
    ...
    public function ajax_send_password_2fa_code() {
        ...
        if ($rate_count >= 3) {
            wp_send_json_error(['message' => 'Muitas tentativas. Aguarde alguns minutos.']);
            return;
        }
    ...
    public function ajax_verify_password_2fa_code() {
        ...
        if (!$code_data) {
            wp_send_json_error(['message' => 'Código expirado. Solicite um novo código.']);
            return;
        }
        ...
        if ($code_data['attempts'] >= 5) {
            delete_transient('hng_password_2fa_' . $user_id);
            wp_send_json_error(['message' => 'Muitas tentativas incorretas. Solicite um novo código.']);
            return;
        }
        ...
        if (!wp_check_password($submitted_code, $code_data['code'])) {
            ...
            wp_send_json_error(['message' => "Código incorreto. {$remaining} tentativas restantes."]);
            return;
        }
    ...
    public function ajax_update_profile() {
        ...
        if (!empty($_POST['client_type'])) {
            ...
            if (in_array($client_type, ['provider', 'company'], true)) {
