...
class HNG_Live_Chat {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_hng_live_chat_long_poll', [$this, 'ajax_long_poll']);
        add_action('wp_ajax_nopriv_hng_live_chat_long_poll', [$this, 'ajax_long_poll']);
        
        // Server-Sent Events handlers
        add_action('wp_ajax_hng_live_chat_sse', [$this, 'ajax_sse_stream']);
        add_action('wp_ajax_nopriv_hng_live_chat_sse', [$this, 'ajax_sse_stream']);
    ...
    public function render_settings_tab() {
        ...
        <div class="hng-live-chat-settings">
            ...
            <h2 style="display: inline-flex; align-items: center;">
                ...
                <?php echo hng_admin_tooltip(
                    ...
                    [
                        'title' => '💡 Dicas',
                        'items' => [
                            ['label' => 'Captura de leads', 'text' => 'Ative email obrigatório'],
                            ['label' => 'Máximo suporte', 'text' => 'Permita visitantes sem email'],
                        ]
                    ]
                ); ?>
            </h2>
            ...
            <h2 style="display: inline-flex; align-items: center;">
                ...
                <?php echo hng_admin_tooltip(
                    ...
                    [
                        ...
                        'items' => [
                            ['label' => 'Equipe pequena', 'text' => 'Marque todos os atendentes'],
                            ['label' => 'Privacidade', 'text' => 'Use nome personalizado genérico'],
                        ]
                    ]
                ); ?>
            </h2>
            ...
            <h2 style="display: inline-flex; align-items: center;">
                ...
                <?php echo hng_admin_tooltip(
                    ...
                    [
                        ...
                        'items' => [
                            ['label' => 'Contraste', 'text' => 'Use cor que destaque do fundo do site'],
                            ['label' => 'Consistência', 'text' => 'Use as cores da sua marca'],
                        ]
                    ]
                ); ?>
            </h2>
            ...
            <h2 style="display: inline-flex; align-items: center;">
                ...
                <?php echo hng_admin_tooltip(
                    ...
                    [
                        'title' => '⚠️ Importante',
                        'items' => [
                            ['label' => 'Fuso horário', 'text' => 'Usa o fuso configurado no WordPress'],
                            ['label' => 'Se desativado', 'text' => 'Chat fica disponível 24/7'],
                        ]
                    ]
                ); ?>
            </h2>
            ...
            <h2 style="display: inline-flex; align-items: center;">
                ...
                <?php echo hng_admin_tooltip_simple(
    ...
    public function ajax_sse_stream() {
        ...
        if (!isset($_GET['nonce']) || !wp_verify_nonce(sanitize_text_field($_GET['nonce']), 'hng_live_chat')) {
        ...
        echo "data: " . wp_json_encode(['status' => 'connected', 'session_key' => $session_key]) . "\n\n";
        ...
        while ((time() - $start_time) < $timeout) {
            ...
            if (!empty($messages)) {
                ...
                echo "event: messages\n";
                echo "data: " . wp_json_encode(['messages' => $formatted]) . "\n\n";
            ...
            if ($session && $session->status === 'closed') {
                ...
                echo "data: " . wp_json_encode(['status' => 'closed']) . "\n\n";
            ...
            if ($is_typing && (time() - $is_typing) < 5) {
                ...
                echo "data: " . wp_json_encode(['typing' => true]) . "\n\n";
        ...
        echo "data: " . wp_json_encode(['last_id' => $current_last_id]) . "\n\n";
