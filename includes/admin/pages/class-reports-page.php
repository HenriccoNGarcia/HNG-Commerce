...
class HNG_Reports_Page {
	...
	public static function render() {
		...
		$funnel               = $conversion_data['conversion_funnel'] ?? array(
			'steps'            => array(),
			'final_conversion' => 0,
		);
		$top_templates        = $conversion_data['top_converting_templates'] ?? array();
		$top_checkouts        = $conversion_data['top_converting_checkouts'] ?? array();
		$abandonment_category = $conversion_data['cart_abandonment_by_category'] ?? array();
		$abandonment_checkout = $conversion_data['cart_abandonment_by_checkout'] ?? array();

		// Get additional metrics
		$sales_metrics  = self::get_sales_metrics( $start_date, $end_date );
		$sales_split    = class_exists( 'HNG_Financial_Analytics' ) ? HNG_Financial_Analytics::get_summary_split( $start_date, $end_date ) : array(
			'site'     => array(
				'revenue' => 0,
				'orders'  => 0,
				'aov'     => 0,
							...
							echo wp_kses_post(
								hng_admin_tooltip(
									...
									array(
										array(
											'title'   => 'O que é?',
											'content' => 'Dashboard avançado de análise de e-commerce. Acompanhe performance detalhada de vendas com métricas em tempo real, incluindo funil de conversão, produtos/categorias de destaque, abandono de carrinhos e análise de origem de dados (site vs gateways).',
										),
										array(
											'title'   => '📊 KPIs Principais',
											'content' => '• Faturamento: Receita total do período com variação percentual<br>• Pedidos: Total de vendas realizadas com tendência<br>• Ticket Médio: Valor médio por pedido (AOV)<br>• Taxa de Conversão: Percentual de visitantes que compraram',
										),
										array(
											'title'   => '📈 Análises Detalhadas',
											'content' => '• Funil de Conversão: Passos do cliente até compra com taxa de queda<br>• Origem dos Dados: Separação site (rastreamento) vs gateways (integração)<br>• Produtos Mais Vendidos: Ranking com quantidade e faturamento<br>• Categorias com Mais Vendas: Performance por categoria com percentual<br>• Abandono de Carrinho: Carrinhos perdidos por categoria, taxa e valor<br>• Performance de Páginas: Páginas gerando mais conversões com taxa',
										),
										array(
											'title'   => '📅 Períodos Disponíveis',
											'content' => '• Hoje: Últimas 24 horas<br>• Últimos 7 dias<br>• Últimos 30 dias (padrão)<br>• Últimos 90 dias<br>• Este ano: Dados do ano calendário',
										),
										array(
											'title'   => '📥 Exportação de Dados',
											'content' => 'Exporte todos os relatórios em formato CSV. Mantém os filtros de período selecionados. Ideal para análises em Excel, Google Sheets ou ferramentas BI.',
										),
									),
									array(
										'icon' => '💡',
										'text' => 'Use o funil de conversão para identificar gargalos de vendas. Monitore abandono de carrinho para estratégias de recuperação (email, cupons). Compare site vs gateways para otimizar canais de vendas.',
									)
								)
							);
						}
				...
				<section class="report-card funnel-card">
					...
					<div class="card-body">
						<div class="funnel-visualization">
							...
							$colors    = array( '#6366f1', '#8b5cf6', '#a855f7', '#d946ef' );
					...
					<div class="card-body">
						<?php if ( ! empty( $top_categories ) ) : ?>
							<div class="categories-chart">
								...
								$cat_colors  = array( '#10b981', '#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6' );
											...
											<tr>
												<td class="col-page">
													<a href="<?php echo esc_url( $template['page_url'] ); ?>" target="_blank" class="page-link">
														...
														<?php
														if ( strlen( $template['page_title'] ) > 45 ) {
															echo '...';}
														?>
												...
												<td class="col-type">
													...
													$type        = $type_config[ $template['page_type'] ] ?? array(
														'label' => $template['page_type'],
														'class' => 'type-other',
													);
													?>
