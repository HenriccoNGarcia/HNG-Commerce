<?php // phpcs:disable Generic.Files.LineEndings.InvalidEOLChar, WordPress.Files.FileName.InvalidClassFileName
/**
...
class HNG_Orders_Page {
	...
	private static function render_list_view() {
		...
		<div class="wrap hng-admin-wrap">
			<h1 class="hng-page-title" style="display: flex; align-items: center;">
						...
						hng_admin_tooltip(
							...
							array(
								array(
									'icon'    => '🔍',
									'title'   => 'Filtros',
									'content' => 'Use os filtros acima da tabela para buscar por status, data, cliente ou ID do pedido.',
								),
								array(
									'icon'    => '📊',
									'title'   => 'Status',
									'content' => '<strong>Pendente:</strong> Aguardando pagamento<br><strong>Processando:</strong> Pagamento confirmado<br><strong>Completo:</strong> Entregue/finalizado<br><strong>Cancelado:</strong> Ordem cancelada',
								),
								array(
									'icon'    => '⚡',
									'title'   => 'Ações em Massa',
									'content' => 'Selecione múltiplos pedidos para alterar status, exportar ou excluir de uma vez.',
								),
							),
							array(
								'title' => '💡 Dicas',
								'items' => array(
									array(
										'label' => 'Clique no pedido',
										'text'  => 'para ver detalhes completos',
									),
									array(
										'label' => 'Exporte para CSV',
										'text'  => 'para análise no Excel',
									),
								),
							)
						)
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										$type_labels = array();
										if ( $has_physical ) {
											$type_labels[] = '📦 ' . __( 'Físico', 'hng-commerce' );
										}
										if ( $has_digital ) {
											$type_labels[] = '💾 ' . __( 'Digital', 'hng-commerce' );
										}
										if ( $has_quote ) {
											$type_labels[] = '📋 ' . __( 'Orçamento', 'hng-commerce' );
										}
										if ( $has_appointment ) {
											$type_labels[] = '📅 ' . __( 'Agendamento', 'hng-commerce' );
										}
										if ( $has_subscription ) {
											$type_labels[] = '🔄 ' . __( 'Assinatura', 'hng-commerce' );
										}
					...
					<table class="widefat striped">
						<tbody>
							...
							<tr>
								...
								<td><?php echo esc_html( ucfirst( str_replace( array( '_', ':' ), array( ' ', ' - ' ), $shipping_method ) ) ); ?></td>
							</tr>
							<?php endif; ?>
	...
	private static function handle_bulk_actions() {
		...
		if ( isset( $status_map[ $bulk_action ] ) ) {
			...
			add_action(
				'admin_notices',
				function () use ( $order_ids, $new_status ) {
